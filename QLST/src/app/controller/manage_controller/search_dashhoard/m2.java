...
public class m2 implements Initializable{
	Connection conn = null;
	ResultSet rs = null;
	PreparedStatement pst = null;
	int index=-1;
	ObservableList<M2> listC;
    ...
    @FXML
    private TableView<M2> table_m2;
    
    @FXML
    private TableColumn<M2, Integer> col_no;

    @FXML
    private TableColumn<M2, Integer> col_id;

    @FXML
    private TableColumn<M2, String> col_pro_name;

    @FXML
    private TableColumn<M2, Integer> col_amount;
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		try {
			...
			String sql1 = "SELECT COUNT(order_id) FROM orders WHERE MONTH(orders.time)=2;";
		...
		try {
			conn = connectDB.ConnectDb();
			String sql2 = "SELECT COUNT(input_id) FROM input WHERE MONTH(input.time)=2";
		...
		try {
			conn = connectDB.ConnectDb();
			String sql2 = "SELECT SUM(input_detail.input_price*input_detail.amount) AS 'total' FROM input_detail WHERE MONTH(time)=2";
			String sql1 = "SELECT SUM(orders.total_price) FROM orders WHERE MONTH(orders.time)=2";
			String sql3 = "SELECT SUM(output_detail.quantity)*input_detail.input_price FROM output_detail,input_detail,ware_house,output WHERE output_detail.wh_id=ware_house.wh_id AND input_detail.input_detail_id=ware_house.input_detail_id AND output_detail.output_id=output.output_id AND Month(output.time)=2";
    ...
    private void barChart() throws SQLException {
		...
		String sql1="SELECT product.pro_name,Sum(orders_detail.quantity) AS 'amount' FROM orders_detail,product,orders WHERE orders_detail.pro_id=product.pro_id AND orders_detail.order_id=orders.order_id and MONTH(orders.time)=2 GROUP BY product.pro_name ORDER BY Sum(orders_detail.quantity) DESC";
		pst = conn.prepareStatement(sql1);
		rs = pst.executeQuery();
		while(rs.next()) {
...
//		----------------------------chart-----------------------------------------------------
	...
	private void barChart1() throws SQLException {
		...
		String sql2="SELECT c.cus_name,SUM(o.total_price) AS 'sales_price' FROM orders o, customer c WHERE o.cus_id=c.cus_id and c.cus_code!=1 and MONTH(o.time)=2 GROUP BY o.cus_id ORDER BY SUM(o.total_price) DESC LIMIT 5";
		pst = conn.prepareStatement(sql2);
		rs = pst.executeQuery();
		while(rs.next()) {
	...
	@FXML
	void tableQuarter1() {
		col_no.setCellValueFactory(new PropertyValueFactory<M2, Integer>("no"));
		col_id.setCellValueFactory(new PropertyValueFactory<M2, Integer>("pro_id"));
		col_pro_name.setCellValueFactory(new PropertyValueFactory<M2, String>("pro_name"));
		col_amount.setCellValueFactory(new PropertyValueFactory<M2, Integer>("amount"));		
		listC = connectDB.getDataM2();
		table_m2.setItems(listC);
	}
}

...
public class quarter_2 implements Initializable{
	Connection conn = null;
	ResultSet rs = null;
	PreparedStatement pst = null;
	int index=-1;
    ObservableList<Quarter_2> listC;
    ...
    @FXML
    private TableView<Quarter_2> table_quarter_2;
    
    @FXML
    private TableColumn<Quarter_2, Integer> col_no;

    @FXML
    private TableColumn<Quarter_2, Integer> col_id;

    @FXML
    private TableColumn<Quarter_2, String> col_pro_name;

    @FXML
    private TableColumn<Quarter_2, Integer> col_amount;
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		try {
			...
			String sql1 = "SELECT COUNT(order_id) FROM orders WHERE QUARTER(orders.time)=2;";
		...
		try {
			conn = connectDB.ConnectDb();
			String sql2 = "SELECT COUNT(input_id) FROM input WHERE QUARTER(input.time)=2";
		...
		try {
			conn = connectDB.ConnectDb();
			String sql2 = "SELECT SUM(input_detail.input_price*input_detail.amount) AS 'total' FROM input_detail,input WHERE QUARTER(input.time)=2 AND input.input_id=input_detail.input_id";
			String sql1 = "SELECT SUM(orders.total_price) FROM orders WHERE QUARTER(orders.time)=2";
			String sql3 = "SELECT SUM(output_detail.quantity)*input_detail.input_price FROM output_detail,input_detail,ware_house,output WHERE output_detail.wh_id=ware_house.wh_id AND input_detail.input_detail_id=ware_house.input_detail_id AND QUARTER(output.time)=2 AND output_detail.output_id=output.output_id";
			pst = conn.prepareStatement(sql2);
			rs = pst.executeQuery();
			if (rs.next()) {
...
//		----------------------------chart-----------------------------------------------------
	...
	@FXML
	void tableQuarter2() {
		col_no.setCellValueFactory(new PropertyValueFactory<Quarter_2, Integer>("no"));
		col_id.setCellValueFactory(new PropertyValueFactory<Quarter_2, Integer>("pro_id"));
		col_pro_name.setCellValueFactory(new PropertyValueFactory<Quarter_2, String>("pro_name"));
		col_amount.setCellValueFactory(new PropertyValueFactory<Quarter_2, Integer>("amount"));		
		listC = connectDB.getDataQuater2();
		table_quarter_2.setItems(listC);

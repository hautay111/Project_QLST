...
public class quarter_4 implements Initializable{
	Connection conn = null;
	ResultSet rs = null;
	PreparedStatement pst = null;
	int index=-1;
    ObservableList<Quarter_4> listC;
    ...
    @FXML
    private TableView<Quarter_4> table_quarter_4;
    
    @FXML
    private TableColumn<Quarter_4, Integer> col_no;

    @FXML
    private TableColumn<Quarter_4, Integer> col_id;

    @FXML
    private TableColumn<Quarter_4, String> col_pro_name;

    @FXML
    private TableColumn<Quarter_4, Integer> col_amount;
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		try {
			...
			String sql1 = "SELECT COUNT(order_id) FROM orders WHERE QUARTER(orders.time)=4;";
		...
		try {
			conn = connectDB.ConnectDb();
			String sql2 = "SELECT COUNT(input_id) FROM input WHERE QUARTER(input.time)=4";
		...
		try {
			conn = connectDB.ConnectDb();
			String sql2 = "SELECT SUM(input_detail.input_price*input_detail.amount) AS 'total' FROM input_detail,input WHERE QUARTER(input.time)=4 AND input.input_id=input_detail.input_id";
			String sql1 = "SELECT SUM(orders.total_price) FROM orders WHERE QUARTER(orders.time)=4";
			String sql3 = "SELECT SUM(output_detail.quantity)*input_detail.input_price FROM output_detail,input_detail,ware_house,output WHERE output_detail.wh_id=ware_house.wh_id AND input_detail.input_detail_id=ware_house.input_detail_id AND QUARTER(output.time)=4 AND output_detail.output_id=output.output_id";
			pst = conn.prepareStatement(sql2);
			rs = pst.executeQuery();
			if (rs.next()) {
    ...
    private void barChart() throws SQLException {
		...
		String sql1="SELECT Sum(orders_detail.quantity) AS 'amount',product.pro_name ,product.pro_id FROM orders_detail,orders,product WHERE orders.order_id=orders_detail.order_id AND orders_detail.pro_id=product.pro_id AND quarter(orders.time)=4 GROUP BY product.pro_name ORDER BY Sum(orders_detail.quantity) DESC";
		pst = conn.prepareStatement(sql1);
		rs = pst.executeQuery();
		while(rs.next()) {
...
//		----------------------------chart-----------------------------------------------------
	...
	private void barChart1() throws SQLException {
		...
		String sql2="SELECT c.cus_name,SUM(o.total_price) AS 'sales_price' FROM orders o, customer c WHERE o.cus_id=c.cus_id and c.cus_code!=1 and quarter(o.time)=4 GROUP BY o.cus_id ORDER BY SUM(o.total_price) DESC LIMIT 5";
		pst = conn.prepareStatement(sql2);
		rs = pst.executeQuery();
		while(rs.next()) {
	...
	@FXML
	void tableQuarter1() {
		col_no.setCellValueFactory(new PropertyValueFactory<Quarter_4, Integer>("no"));
		col_id.setCellValueFactory(new PropertyValueFactory<Quarter_4, Integer>("pro_id"));
		col_pro_name.setCellValueFactory(new PropertyValueFactory<Quarter_4, String>("pro_name"));
		col_amount.setCellValueFactory(new PropertyValueFactory<Quarter_4, Integer>("amount"));		
		listC = connectDB.getDataQuater4();
		table_quarter_4.setItems(listC);

...
public class quarter_1 implements Initializable{
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		try {
			conn = connectDB.ConnectDb();
			String sql2 = "SELECT SUM(input_detail.input_price*input_detail.amount) AS 'total' FROM input_detail,input WHERE QUARTER(input.time)=1 AND input.input_id=input_detail.input_id";
			String sql1 = "SELECT SUM(orders.total_price) FROM orders WHERE QUARTER(orders.time)=1";
			String sql3 = "SELECT SUM(output_detail.quantity)*input_detail.input_price FROM output_detail,input_detail,ware_house,output WHERE output_detail.wh_id=ware_house.wh_id AND input_detail.input_detail_id=ware_house.input_detail_id AND QUARTER(output.time)=1 AND output_detail.output_id=output.output_id";
			pst = conn.prepareStatement(sql2);
			rs = pst.executeQuery();
			if (rs.next()) {
